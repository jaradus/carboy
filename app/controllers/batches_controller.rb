class BatchesController < ApplicationController
  ...
  def update
    params[:batch].each do |hop|
      if hop[1]["used"] == "1"
        hop_id =           hop[1]["hop_id"]
        batch_id =         params[:id]   
        hop_id =           hop[1][:hop_id]
        quantity =         hop[1][:quantity]
        unit_measure =     hop[1][:unit_measure]
        minutes_in_boil =  hop[1][:minutes_in_boil]
        alpha_acid =       hop[1][:alpha_acid]
        beta_acid =        hop[1][:beta_acid]
        BatchesHop.create(batch_id: batch_id, hop_id: hop_id, hop_id: hop_id, batch_id: batch_id, hop_id: hop_id, quantity: quantity, unit_measure: unit_measure, minutes_in_boil: minutes_in_boil, alpha_acid: alpha_acid, beta_acid: beta_acid)
      end
    end
